<div class="modal fade" id="modal-edit" tabindex="-1" aria-labelledby="exampleModalXlLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-xl">
           <div class="modal-content">
               ...
               <form action="{{ route('admin.roles.update')}}" method="POST" enctype="multipart/form-data" id="edit-form">
                @csrf
                @method('PUT')
   ...
   <script>
        function selectAllPermissions() {
            let checkboxes = $('input[type="checkbox"].permission');
            let allChecked = checkboxes.length === checkboxes.filter(':checked').length;

            if (allChecked) {
                checkboxes.prop('checked', false);
                toastr.info('Todos los permisos han sido deseleccionados.');
            } else {
                checkboxes.prop('checked', true);
                toastr.success('Todos los permisos han sido seleccionados.');
            }
        }
    </script>
   @stop
