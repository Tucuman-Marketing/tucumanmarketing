<script>
    ...
    //Permisos
    var permissions = {
        canDelete: @permission('user-delete') true @else false @endcan,
        canEdit: @permission('user-edit') true @else false @endcan,
        canShow: @permission('user-view') true @else false @endcan,
    };
    //Urls
    var urls = {
        baseUrl: "{{ asset('') }}",
        baseUrlShow: "{{ route('admin.users.show', ['object' => 'tempId']) }}",
        baseUrlEdit: "{{ route('admin.users.edit', ['object' => 'tempId']) }}",
        baseUrlDelete: "{{ route('admin.users.destroy', ['object' => 'tempId']) }}",
        baseUrlMassDelete: "{{ route('admin.users.massDestroy') }}",
        indexUrl: "{{ route('admin.users.indexDatatable') }}"
    };
    //Ordenamiento
    var order = [
        [3, 'desc']
