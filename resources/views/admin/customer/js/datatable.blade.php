...
var table;
var baseUrl             = "{{ asset('') }}";
var baseUrlShow         = "{{ route('admin.customers.show', ['customer' => 'tempId']) }}";
var baseUrlEdit         = "{{ route('admin.customers.edit', ['customer' => 'tempId']) }}";
var baseUrlDelete       = "{{ route('admin.customers.destroy', ['customer' => 'tempId']) }}";
var getSubSectorsUrl    = null;
var csrfToken           = '{{ csrf_token() }}';
/*============================PERMISSIONS============================*/
var canDelete  = @permission('client-delete') true @else false @endcan;
var canEdit     = @permission('client-edit') true @else false @endcan;
var canShow     = @permission('client-view') true @else false @endcan;
/*============================PERMISSIONS============================*/
...
function activartabla() {
    ...
    table = $('#datatable').DataTable({
        ...
        pageLength: 50,
        order: [[ 8, "desc" ]],//ordenamiento por columna 0
        searching: true,
        fixedHeader: true,
        deferRender: true,
        responsive: false,
        ...
        /*============================AJAX============================*/
        ajax: "{{ route('admin.customers.indexDatatable') }}",
        /*============================AJAX============================*/
        ...
        columns: [
            ...
            //imagen
            {
                ...
                orderable: false,
                render: function(data, type, row) {
                    var imageUrl = row.image ? baseUrl + row.image : baseUrl + 'theme-admin/volgh/assets/images/icon/user.svg';
            ...
            //LASTNAME
            {
                ...
                orderable: true,
                render: function(data, type, row) {
                    var html = '<span>' + row.lastname + '</span>';
                    return html;
                }
            },
            //DNI
