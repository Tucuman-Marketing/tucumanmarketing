<script>
    var tableName = 'recruitment_candidates_by_job_table';
    var ajaxMethod = 'GET';
    var jobUuid = '{{ $data->uuid }}';
    ...
    var urls = {
        ...
        indexUrl: "{{ route('admin.recruitment.jobs.candidatesByJob.indexDatatable', ['jobUuid' => 'tempJobUuid']) }}".replace('tempJobUuid', jobUuid)
    };
    //Ordenamiento
    var order = [
        [3, 'desc']
...
function openStatusModal(row) {
    // Llena el campo oculto con los datos del objeto
    $('#modalId').val(row.id);
    // Selecciona el radio button correspondiente al estado
    if (row.status) {
        $('input[name="status_id"][value="' + row.status + '"]').prop('checked', true);
    } else {
        // Desmarcar todos los radio buttons si no hay un estado específico
