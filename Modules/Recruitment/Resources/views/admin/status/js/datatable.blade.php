<script>
    var tableName = 'recruitment_status_table';
    var ajaxMethod = 'GET';
    //Permisos
    var permissions = {
        canDelete: @permission('recruitment-status-delete') true @else false @endcan,
        canEdit: @permission('recruitment-status-edit') true @else false @endcan,
        canShow: @permission('recruitment-status-view') true @else false @endcan,
    };
    //Urls
    var urls = {
        baseUrl: "{{ asset('') }}",
        baseUrlShow: "{{ route('admin.recruitment.status.show', ['object' => 'tempId']) }}",
        baseUrlEdit: "{{ route('admin.recruitment.status.edit', ['object' => 'tempId']) }}",
        baseUrlDelete: "{{ route('admin.recruitment.status.destroy', ['object' => 'tempId']) }}",
        baseUrlMassDelete: "{{ route('admin.recruitment.status.massDestroy') }}",
        indexUrl: "{{ route('admin.recruitment.status.indexDatatable') }}"
    };
    //Ordenamiento
    var order = [
        [3, 'desc']
