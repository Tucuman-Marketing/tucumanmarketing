<script>
    var tableName = 'blog_status_table';
    var ajaxMethod = 'GET';
    //Permisos
    var permissions = {
    ...
    var urls = {
        baseUrl: "{{ asset('') }}",
        baseUrlShow: "{{ route('admin.blogs.category.show', ['category' => 'tempId']) }}",
        baseUrlEdit: "{{ route('admin.blogs.category.edit', ['category' => 'tempId']) }}",
        baseUrlDelete: "{{ route('admin.blogs.category.destroy', ['category' => 'tempId']) }}",
        baseUrlMassDelete: "{{ route('admin.blogs.category.massDestroy') }}",
        indexUrl: "{{ route('admin.blogs.category.indexDatatable') }}"
    };
    //Ordenamiento
    var order = [
        [3, 'desc']
